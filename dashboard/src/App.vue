...
export default {
 ...
 data() {
   return {
     apiKey: '546e81701cd7e31a38e744935da1573f',
     weatherDetails: false,
     location: 'La playita, Punta Lara', // raw location from input
     lat: '-34.825466', // raw latitude from google maps api response
     long: '-57.961466', // raw longitude from google maps api response
 ...
 methods: {
   ...
   fahToCel: function(tempInFahrenheit) {
    ...
    kelToCel: function(tempInKelvin) {
   ...
   mileToKnot: function(miles) {
     var knot = miles * 0.868976;
     return Math.round(knot);
   },
   ...
   locationEntered: function() {
     var input = this.$refs.input;
     if (input.value === '') {
       this.location = "La playita, Punta lara";
     } else {
   ...
   getSetTodayTempHighLowWithTime: function() {
     ...
     this.currentWeather.todayHighLow.todayTempHigh = this.kelToCel(
     ...
     //).onlyTime;
     this.currentWeather.todayHighLow.todayTempLow = this.kelToCel(
